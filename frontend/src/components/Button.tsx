...
const Button = ({ text, color, link }: Props) => {
  return (
    <Link to={link}>
      {" "}
      <button
        className={` w-42 flex items-center gap-3 rounded-sm border px-6 py-3 text-2xs font-semibold uppercase  tracking-widest transition-colors duration-300  md:px-8  ${
          color === "red"
